...
if uploaded:
    ...
    if mode == "Reverse CDF Only":
        ...
        # P10, P50, P90 - CORRECTED for exceedance
        p10 = np.percentile(ooip_values, 90)  # P10 = 90th percentile (10% exceedance)
        p50 = np.percentile(ooip_values, 50)  # P50 = 50th percentile (50% exceedance)
        p90 = np.percentile(ooip_values, 10)  # P90 = 10th percentile (90% exceedance)
        fmt = f"{{:.{decimals}e}}"
        # ------------------------------------------------------------------ #
        # Matplotlib Descending CDF (Exceedance) Plot - CORRECTED
        # ------------------------------------------------------------------ #
        ...
        ax.invert_xaxis()  # FIXED: Invert x-axis for reverse CDF
    ...
    if st.button("Run Monte Carlo Simulation", type="primary", use_container_width=True):
        with st.spinner("Simulating..."):
            ...
            # CORRECTED: P10 = 90th percentile, P90 = 10th percentile
        ...
        if "Plotly" in plot_engine:
            ...
            # FIXED: Reverse x-axis for descending CDF
            ...
            # FIXED: Descending CDF with inverted x-axis
            ...
            ax3.invert_xaxis()  # FIXED: Invert x-axis
            for val, label, color in [(p10, "P10", "#59a14f"), (p50, "P50", "#f28e2b"), (p90, "P90", "#e15759")]:
                val_str = f"{val:.{decimals}e}"
            ...
            st.download_button("Download Log Histogram", buf4, "log_histogram.png")
            plt.close('all')
        # ----------------------------------------------------------------
        # 8. Download Results
        # ----------------------------------------------------------------
